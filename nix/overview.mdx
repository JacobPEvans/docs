...
The four Nix repos are layered, not parallel. `nix-darwin` sits at the top and orchestrates the macOS system. `nix-home` defines the user environment via home-manager. `nix-ai` packages every AI coding tool — Claude, Antigravity, Copilot, MLX, MCP servers. `nix-devenv` provides reusable project-level dev shells.
