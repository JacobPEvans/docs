...
description: "Pool names encode the storage medium (nvme / ssd / sas / bulk) and nothing else — never the node, the RAID level, or the drive count — so dataset paths are identical on every node and replicas line up by construction."
page_type: decision
---
...
A node only creates the pools it has hardware for. A `proxmox-2`-style node with an HDD array gets `bulk`; a node with a spare SATA SSD gets `ssd`; a node with a spare NVMe device gets `nvme`; every ZFS-boot node already has `rpool`.
