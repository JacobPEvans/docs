...
A node only creates the pools it has hardware for. A `proxmox-2`-style node with an HDD array gets `bulk`; a node with spare enterprise spinning disks gets `sas`; a node with a spare SATA SSD gets `ssd`; a node with a spare NVMe device gets `nvme`; every ZFS-boot node already has `rpool`.
