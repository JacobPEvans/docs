...
| App-consistent backup | **Logical dumps** | Per-application dump jobs (for example `pg_dump`) for data that must be consistent mid-write, written to a replicated dataset |
| Capacity alerting | **ntfy** | Pushes a notification when a pool crosses 50% / 75% / 90% |

Snapshots and replication protect the filesystem; logical dumps protect the *applications* (a database mid-transaction needs an application-consistent backup, not just a block snapshot). The two are complementary, not redundant. A dedicated backup appliance was evaluated for this layer and deliberately not adopted — the dump-plus-replication pattern already covers the gap without operating a second backup product.
...
keeps snapshotting the **old** path. The old dataset usually still exists — a
tier move commonly retains the source — so snapshots keep being created and
...
- If a guest's storage cannot be resolved to a dataset — a directory-backed
  storage, for example, which has no per-guest dataset — the render must fail
