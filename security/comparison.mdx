...
- **Secret-zero lives one tier up.** T2 cannot bootstrap itself from nothing: its static seal key and the AppRole `secret_id` that agents use to authenticate are held in T3. That, plus the handful of values an external service reads on its own, is the whole of T3 — everything else lives in T2 or is minted by an engine on demand. T3 is therefore small, tightly scoped, and only AI-reachable under human approval. T4 holds the human-only recovery material that can rebuild everything if both T2 and T3 are lost.
...
**aws-vault** and the automated use of **macOS Keychain** are retired from the machine path. T2's AWS secrets engine and token-file reads replaced them; a human at a laptop may still keep a local session, headless agents have none.
