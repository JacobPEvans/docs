...
Everything else lives in T2, where credentials are self-hosted, short-lived, and readable with zero interactive prompts, or is minted on demand by an engine and never stored at all.
...
A value that lands here by habit rather than by one of the two reasons above is
drift. It is migrated out, and the migration counts as finished only when a live
