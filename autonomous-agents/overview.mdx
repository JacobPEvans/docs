...
| **Google Gemini** | `--approval-mode yolo` | [Sandbox is opt-in](https://geminicli.com/docs/cli/sandbox/) and not coupled to yolo — the weakest pairing of the three, and the only tool that cannot persist yolo in settings at all, so the coupling is enforced by the launcher and the sandbox around it. |
...
One exception breaks the "configs only, no flags" shape: Gemini strips the
settings key that would persist yolo, so every autonomous Gemini run must pass
`--approval-mode yolo` on the command line, and the launcher asserts the flag is
present. The rendered settings block for Gemini exists for parity, not effect.
