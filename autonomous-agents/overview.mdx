...
entries rendered into Claude Code, Codex CLI, and the Antigravity (`agy`) CLI — running on a trusted
...
| **Antigravity (`agy`)** | `--approval-mode yolo` | [Sandbox is opt-in](https://geminicli.com/docs/cli/sandbox/) and not coupled to yolo — the weakest pairing of the three, and the only tool that cannot persist yolo in settings at all, so the coupling is enforced by the launcher and the sandbox around it. |
...
| `autonomous` | inside containers ONLY | `bypassPermissions` + ~10-entry residual deny; `claude -p --bare --dangerously-skip-permissions` | `approval_policy="never"`, `sandbox_mode="danger-full-access"` (the container IS the sandbox; bwrap can't nest) | `--approval-mode yolo`, its own sandbox off |
| `ci` | Actions runners | `autonomous` + JSON output overlay | same | same |

One exception breaks the "configs only, no flags" shape: Antigravity strips the
settings key that would persist yolo, so every autonomous `agy` run must pass
`--approval-mode yolo` on the command line, and the launcher asserts the flag is
present. The rendered settings block for it exists for parity, not effect.
