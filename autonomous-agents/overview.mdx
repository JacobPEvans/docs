...
| **Antigravity (`agy`)** | `--dangerously-skip-permissions` | Its own sandbox is disabled and the posture is not coupled to it — the weakest pairing of the three, and the only tool that cannot carry the posture as a settings key at all, so the coupling is enforced by the launcher and the sandbox around it. |
