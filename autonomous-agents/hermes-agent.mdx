...
- **Brain:** an always-on local GPU model (OpenAI-compatible), so the agent never
  ...
  resident model** served behind the router and selected at runtime from a value
  in the secret store, so re-pointing it needs no rebuild and no agent restart.
  It runs with **thinking ON** and carries sampling guardrails for quantized
  brains.
