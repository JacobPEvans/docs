...
- **Containment:** the LXC is the blast-radius boundary, with deliberately narrow
  ...
  job gets — a tool/credential boundary, not an OS sandbox: every profile runs as
...
each its own container, Slack app, OpenBao credential domain, and memory bank, so
one agent's state never leaks into another's. The primary identity is `hermes`;
a second, purpose-built identity (`donna`) runs the same way. New identities are
declared, not layered on top of an existing one — see
...
No arrow connects the two containers above — that is the point. Separate agents
share no memory, credentials, or Slack channel.
...
sole chat platform in front of it today.
