...
description: "Running herdr as a homelab service: one runtime holds every coding agent's terminal, a Slack bridge answers blocked agents, and a web dashboard approves them — none of it tied to a laptop."
...
through a CLI and a local socket API.
...
Each part runs in its own LXC. A crash in the Slack bridge must not touch the
...
| --- | --- | --- |
| Runtime | Holds the agent panes and every coding CLI | No — SSH only |
...
The agent CLIs used to come from Homebrew. Homebrew is macOS-only, so the stack
could not leave one laptop — which is the whole problem this solves.
...
CLIs the main set does not carry. Homebrew is kept only for desktop apps, which
have no Nix equivalent.

## Exposure

The dashboard is the only part reachable from a browser, and it sits behind the
...
declared in code, that is state arriving from outside. Local definitions win,
so declare the ones that matter rather than trusting whatever the network last
sent.
