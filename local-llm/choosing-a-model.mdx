...
The leading `2` is the key and the value cache. Worked example with Llama-3.3-70B
(`num_hidden_layers` 80, `num_key_value_heads` 8, `head_dim` 128,
`max_position_embeddings` 131072 — values identical across three independent
mirrors of the config, since Meta's own repo is gated):

```text
2 × 80 × 8 × 128 × 2 bytes = 327,680 bytes/token = 0.3125 MiB/token
327,680 × 131,072          = 42,949,672,960 bytes = exactly 40 GiB
```

Forty GiB of cache, on top of the weights, on a machine with ~95 GiB usable. That
is the line item people forget.

**Architecture changes this enormously.** DeepSeek-V2's multi-head latent
attention (MLA) compresses the KV cache into a low-rank latent vector; the paper
reports a **93.3% KV cache reduction** versus DeepSeek 67B
([arXiv:2405.04434](https://arxiv.org/abs/2405.04434)). Hybrid and
linear-attention designs go further: the Jamba paper's Table 1 puts KV cache at
256K context at 128 GB for LLaMA-2, 32 GB for Mistral and Mixtral, and **4 GB for
Jamba** ([arXiv:2403.19887](https://arxiv.org/abs/2403.19887)). Two models with
the same parameter count can differ by an order of magnitude in cache cost.
...
<Steps>
  ...
  <Step title="Compute the KV budget for your real target context">
    Use the formula above with the model's actual `num_key_value_heads` and
    `head_dim`, at the context length you will really run — not the advertised
    maximum. Add it to the weight size before deciding anything fits.
  </Step>
  ...
  <Step title="Verify what is actually loaded from the serving process">
    Not from the API. See below.
  </Step>
</Steps>

### How to verify what a server is actually running

[Verifying the instrument](/local-llm/verifying-the-instrument) covers this in
full, including a case where an unverified proxy produced a 7× measurement error.
The short version, with the source-level proof:

The `model` field in an OpenAI-compatible response is **an echo of your request**,
not a report of what was loaded. In `mlx_lm/server.py` the handler sets
`self.requested_model = self.body.get("model", "default_model")` and then emits
`"model": self.requested_model` in the response body. A swapping proxy in front
of several workers can therefore answer, with HTTP 200, to a name it never loaded.

Resolve the listening port to a process and read its command line instead
