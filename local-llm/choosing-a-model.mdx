...
<Warning>
  **Recompute this from `config.json` yourself. Do not trust any published KV
  ...
  hand — silently doubles a row, and a review pass alone does not catch it.
...
| --- | --- |
| **Alias grafting on collapse** *(fixed — kept as a failure pattern to recognise)* | A swap-style proxy collapses to serving one model. The other catalogue entries move to a disabled list — and in the affected implementation **their physical model ids were grafted onto the surviving entry as aliases**, so "every known name still gets an answer": requesting a disabled model's id resolved, to the surviving model's weights. The fix is that a disabled id **returns 404** instead. The pattern is worth recognising in any proxy that collapses a catalogue, because it fails with a 200 and the requested name echoed back. |
