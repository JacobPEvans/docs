...
description: "How the prompt cache, concurrency and the paged KV pool compete for one memory budget on a local MLX server — why KV quantization and concurrency are mutually exclusive on the serving path, why one number must feed both the admission cap and the engine's batch width, and why an over-long context window fails on cache starvation rather than on RAM."
...
> prompt cache absorbs whatever is left — which is why the context window that
...
more expensive than a short one, and it is paid on **every** turn once the
...
against 8.34 s cold after a single intervening conversation at 7k tokens. Treat
that ratio as **directional evidence that eviction is expensive**, not as a
coefficient: it is a single sample on one machine, and the penalty grows with
context.
</Note>

The rule that follows: give each concurrent stream at least its own slot, and
size the cache so a slot can hold the window it is caching for. A slot too small
...
serving path, that option is closed — and not merely because the flags are
...
are mutually exclusive**, and adding the CLI flags would not change that —
...
buffers. Frameworks impose a **maximum buffer count**, and exceeding it fails
...
architecture), 17 GiB of weights, two concurrent streams, and a 48 GiB
...
memory exhaustion.** A sizing check that only asks "does it fit?" will happily
recommend the slower configuration.

## Derive these, do not hand-tune them

Every quantity here is a function of a small number of inputs: the model's
architecture, the per-worker budget, and two choices — concurrency and window.
