...
description: "Two 128 GB Macs serve one model sharded across both over Thunderbolt 5 with JACCL RDMA — nix-managed, live, and reboot-survivable. The re-enable procedure, the fixed-vs-in-progress root causes, and the honest mechanism."
...
TB5 MLX clustering is **the point of running two 128 GB Macs** — it's what
turns two separate boxes into one serving fabric that can hold a model neither
can hold alone. It works: JACCL RDMA over Thunderbolt 5 forms reliably on
current hardware and macOS 26.2+, plenty of setups run it, and this fleet's
own cluster is nix-managed, live, and survives a reboot. This page is the
architecture and the re-enable procedure; the fixes that got it there are a
root-cause table, not a warning label.
...
The full config reference — file-by-file module map, rank/host assignment,
memory-sizing math — lives in the private operational reference (see
**Related** below); this page stays the public-safe architecture and
...
Measure residency with `vm_stat`'s wired-memory figure, never a process's RSS.
...
model qualifies, and which mode a given clusterable model actually runs in is
...
whose weights leave each node with cache headroom rather than pushing toward
the notional 256 GB ceiling.

## Capacity, and where the throughput lands

Sharded cross-Mac inference is communication-bound, so the win is capacity —
running a model that fits nowhere else — not raw speed. Measured on this
fleet's cluster, GLM-4.7-Flash (`glm4_moe`, pipeline-parallel, hand-run
...
| Warmup slot starvation caused hourly 429s under load | **Fixed** — repair caps and halt bounds | landed |
| macOS re-enslaves the Thunderbolt port into `bridge0` after a reboot, starving RDMA | **Fixed** — boot daemon; verified at converge-time, reboot-time verification pending the next natural reboot | [dryvist/nix-darwin#2073](https://github.com/dryvist/nix-darwin/pull/2073) (nix-darwin#1768) |
