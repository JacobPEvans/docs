...
<Note>
  This mechanism has since been **fixed** in the estate it was observed in: the
  grafting is banned in the config generator, and a disabled model's physical id
  now returns **HTTP 404**. It is kept here because the *shape* generalises to
  any proxy that collapses a catalogue, and because it fails in the worst
  possible way — a 200 with the requested name echoed back.
</Note>
...
generator did one more thing: it grafted every disabled model's **physical
...
it. Ask for a 120B model, get the 30B resident — HTTP 200, and a response
labelled with the 120B you asked for. In one captured config, 11 of 12 model ids
resolved to the same resident.
...
runs — a spread wide enough to swallow the entire effect being attributed. A
ratio built from two single runs is a ratio of two samples from overlapping
distributions, so it identifies nothing, and the confident decimal places make
...
- **Restart between arms.** Most serving options are read at startup only, so an
  edited config without a restart benchmarks the *old* setting — and returns a
  clean null result that looks like evidence.
...
<CardGroup cols={2}>
  ...
  <Card title="Speculative decoding" icon="forward-fast" href="/local-llm/speculative-decoding">
