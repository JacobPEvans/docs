...
<Warning>
  **Numbers below are provisional until re-measured with process-proven
  weights.** The swapping proxy was found serving a fallback model's weights
  under other models' labels, which inflated one measurement by 7x. Any figure
  not taken from a worker whose command line was inspected is under audit — see
