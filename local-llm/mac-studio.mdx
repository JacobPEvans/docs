...
Verified 2026-07-26/27. Both figures come from a dedicated worker whose loaded
weights were confirmed from its running command line. The headline metric is
**cumulative** — see
[the metric definition](/tools/mlx-benchmarks#the-headline-metric-is-cumulative-not-decode-only).

| Model | Cumulative | Decode-only | Prefill | Time to first token |
| --- | --- | --- | --- | --- |
| `gpt-oss-120b-MXFP4-Q8` | **51–54 tok/s** | 40–43 tok/s | 559–627 tok/s | 0.23–0.26 s |
| `Qwen3-Next-80B-A3B-Thinking-4bit` | **10–13 tok/s** | 9–11 tok/s | 97–152 tok/s | 0.58–0.91 s |

About a 4.5x gap. The 80B's slowness is an implementation property of its
hybrid-attention path, not a size property — the larger model is the faster one
here. Storage and memory pressure were both ruled out.
