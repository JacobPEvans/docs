...
- **Fast tier** — small, quick models (`qwen3-4b` and the
  ...
  large tier as a fallback leg — **fail-fast, not retrying**: it accepts a
...
by a learned quality-vs-cost tradeoff rather than a fixed order — see
[Cost as data](/ai-development/model-routing#cost-as-data-not-a-hardcoded-order).
Each tier still registers an honest cost, so in practice the order above is
...
Teal is a client, ink is the DNS + reverse-proxy edge, coral is the local
serving fabric, dashed teal is a hosted tier reached only as fallback or by
explicit request. Traefik health-checks the router pool
(`/health/liveliness`) and drops a dead router without the endpoint
changing — stopping one router live is a non-event. Every name resolves
through Technitium and carries the wildcard certificate, so it is HTTPS end
to end.
