...
<Warning>
  **Every figure above is provisional until re-measured with process-proven
  weights.** The swapping proxy was caught serving a fallback model's weights
  under other models' labels, inflating one measurement by 7x, and the
  concurrency aggregate is not a general result — on a large MoE model, four
  streams measured *worse* than one (verified 2026-07-26/27). Read
  [Verifying the instrument](/local-llm/verifying-the-instrument) before citing
  any number on this site.

  They are also quoted on the wrong metric. The headline figure is now
  **cumulative** tokens/second — prompt plus completion over total wall-clock —
  because decode-only rates hide real prefill gains. See
