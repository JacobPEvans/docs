---
title: "Local LLM"
description: "One strategy for local inference: a fast resident model on each client laptop, and an always-on desktop-class Apple Silicon host serving the large tier to the whole LAN — laptops as clients."
page_type: overview
---
...
<Warning>
  **Why those figures were pulled, and why you should distrust any number that
  lacks its provenance.** The swapping proxy was caught serving the resident
  model's weights under every other model's label — single-model collapse
  grafts each disabled model's *physical identifier* onto the surviving entry
  as an alias — which
  inflated one measurement by 7x. The concurrency aggregate is not a general
...
- **A small resident set, not a rotation.** Swapping a multi-GB model evicts
  wired GPU memory and reloads another — the slowest thing you can do. A laptop
  holds one resident model; the serving host holds two. Capability-role aliases
  (`default`, `coding`, `quickest`, `tool-calling`, …) resolve onto that
