...
<Warning>
  **Every figure above is provisional until re-measured with process-proven
  weights.** The swapping proxy was caught serving the resident model's weights
  under every other model's label — single-model collapse *used to* graft each
  ...
  model's physical id is no longer aliased and now returns HTTP 404.** The
  figures taken while it was live remain provisional regardless. The
  ...
  They are also quoted on the wrong metric, and without a regime. The headline
