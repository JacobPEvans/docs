...
<Warning>
  **Why those figures were pulled, and why you should distrust any number that
  ...
  disabled model's *physical identifier* onto the surviving entry as an alias —
  which inflated one measurement by 7x. That defect is fixed: a disabled
  model's physical id no longer gets aliased, and now returns HTTP 404. The
