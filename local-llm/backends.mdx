...
with the channel-markup case above, and the parser was written as a declarative
patch rather than the model being written off.
...
<Warning>
  **Do not justify a parser on a throughput claim you have not re-measured.**
  ...
  2.6x — the opposite direction.

  The parser is still correct and still worth having: the markup leakage is real
  and independently measured on an isolated worker. It just needed a reason that
  survived scrutiny, and "fixes a genuine plumbing defect" was always the
