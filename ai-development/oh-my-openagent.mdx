...
agent harness that loads into an existing CLI host. Three editions ship from
the same project:
...
| **Light** | Codex CLI | `bunx lazycodex-ai install` | `~/.codex/plugins/cache/sisyphuslabs/`, `~/.codex/agents/`, `~/.local/bin` CLIs, `config.toml` blocks |
...
which fails against the read-only store and would be clobbered by the next
...
and the next rebuild would revert the installer's danger-full-access keys into
a mixed posture.
...
its doctor flags the installer-generated `models[]` arrays as invalid while
calling the documented `model`/`fallback_models` shape deprecated, and its own
`config migrate` is a no-op. Either shape loads (exit 0); the normalized shape
