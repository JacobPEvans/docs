...
Situational agent guidance — pre-commit architecture, the Nix tool policy, shared-workflow org references — ships as
on-demand skills in [claude-code-plugins](/ai-development/claude-code-plugins) instead of always-loaded rules, keeping
the per-session footprint focused on voice, tool use, skill-execution integrity, and repository-local routing.
