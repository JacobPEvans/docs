...
The [LiteLLM router pool](/local-llm/homelab-gpu) is the multi-provider gateway in front of workstation AI tools, fanning requests out to OpenAI, Gemini, OpenRouter, and local MLX inference based on the task class.
...
2. **LiteLLM router** — multi-provider routing across the router pool
